...
def prefill_attention_fa2():
    from flash_attn import flash_attn_varlen_func as _fa2_varlen

    class Fa2Attention(torch.autograd.Function):
        @staticmethod
        def forward(ctx, q, k, v, num_q_heads, num_kv_heads, head_dim, warmup=10):
            seq_len = q.shape[0]
            scale = head_dim ** -0.5
            qo_indptr = torch.tensor([0, seq_len], dtype=torch.int32, device=q.device)
...
def main(args):
    ...
    attn_fa3 = prefill_attention_fa2().apply
